...
var roleHarvester = {
    ...
    run: function (creep) {
        if (creep.memory.status == 0) {
            if (creep.hits < 1500) {
                creep.say("I'm sorry to harvest the Deposit");
                creep.suicide();
            }
            var mineral1 = new RoomPosition(35, 9, 'E40N55');
            if (creep.room.name != 'E40N55') {
                creep.moveTo(mineral1);
            }
            else {
                if (creep.room.name == 'E40N55') {
                    sources1 = Game.getObjectById('602153dfb2e692a8ab080661');
                    if (sources1.cooldown ==0 ) {
